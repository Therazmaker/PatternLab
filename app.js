...
async function init() {
  ...
  microBotTab = createMicroBotTab({
    elements: {
      ...
      diagnosticSummary: els.microBotDiagnosticSummary,
    ...
    document.getElementById("gemini-neuron-modal-container"),
    {
      onSave: (item) => {
        ...
        geminiBotController?.logBrainEvent?.({
          timestamp: new Date().toISOString(),
          type: "neuron_saved",
          patternName: item?.patternType || item?.name || "manual_neuron",
          outcome: null,
          reasonCode: "manual_save",
          details: "neuron saved from modal",
          meta: { neuronId: item?.id || null },
        });
      },
      onDelete: (id) => {
