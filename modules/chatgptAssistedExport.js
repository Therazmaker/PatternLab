...
export function buildChatGPTAssistedExport(currentSessionContext = {}) {
  ...
  const payload = {
    ...
    context_score: asNumber(brainVerdict?.context_score),
    danger_score: asNumber(brainVerdict?.danger_score),
    familiarity: asNumber(brainVerdict?.familiarity),
    learned_bias: brainVerdict?.learned_bias || null,
    last_outcomes: compactList(brainVerdict?.last_outcomes || [], 8),
    decision_path: compactObject(brainVerdict?.decision_path || {}),
